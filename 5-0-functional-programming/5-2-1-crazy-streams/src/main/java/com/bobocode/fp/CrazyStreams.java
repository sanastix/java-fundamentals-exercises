...
public class CrazyStreams {
    ...
    public BigDecimal calculateTotalBalance() {
        return BigDecimal.valueOf(accounts.stream().mapToInt(account -> account.getBalance().intValue()).sum());
    }
    ...
    public BigDecimal getBalanceByEmail(String email) {
        return accounts.stream().filter(account -> account.getEmail().equals(email)).map(Account::getBalance).findFirst().orElseThrow(() -> new EntityNotFoundException("Cannot find Account by email=" + email));
    }
    ...
    public Map<Month, String> groupCommaSeparatedFirstNamesByBirthdayMonth() {
        return accounts.stream().collect(Collectors.groupingBy(account -> account.getBirthday().getMonth(), Collectors.mapping(Account::getFirstName, Collectors.joining(", "))));
    }
